...
#modify
#player.asm - the spaceship that the player controls.
...
enter
	...
	# will call each (non-empty) object’s update method. 
	#These methods are listed in globals.asm, in the object_update_funcs array.
	jal player_collide_all
	#will test for collision between the player and any object that has a non-null entry in the

	# draw
	jal Object_draw_all
	#call each (non-empty) object’s draw method, if it has one.
	jal hud_draw
